...
import org.scapesoft.engine.process.impl.PunishmentProcessor;
 ...
 * @since Mar 24, 2014
 */
public class PunishmentLoader extends GsonLoader<Punishment> {
		...
		synchronized (PunishmentProcessor.LOCK_OBJECT) {
			...
			while (it.hasNext()) {
				...
				if (punishment.getType() == type) {
					if (key.replaceAll("_", " ").equalsIgnoreCase(punishment.getKey().replaceAll("_", " "))) {
