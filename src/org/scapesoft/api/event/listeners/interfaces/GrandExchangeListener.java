...
import static org.scapesoft.game.player.content.exchange.ExchangeConfiguration.COLLECTION_INTERFACE;
import static org.scapesoft.game.player.content.exchange.ExchangeConfiguration.MAIN_INTERFACE;
import static org.scapesoft.game.player.content.exchange.ExchangeConfiguration.SELL_INTERFACE;
...
import org.scapesoft.engine.tasks.ExchangeTask;
...
public class GrandExchangeListener extends EventListener {
	...
	public boolean handleButtonClick(final Player player, int interfaceId, int buttonId, int packetId, int slotId, int itemId) {
		...
		case MAIN_INTERFACE:
			...
			case 175: // set guide price
				...
				if (offer == null)
					break;
				offer.setPrice(((ExchangePriceLoader) GsonHandler.getJsonLoader(ExchangePriceLoader.class)).getAveragePrice(offer.getItemId()));
				player.getPackets().sendConfig(1111, offer.getPrice());
				break;
			...
			case 186: // confirm
				...
				case BUY:
					if (player.takeMoney(requestedCash)) {
						...
						ExchangeItemLoader loader = GsonHandler.getJsonLoader(ExchangeItemLoader.class);
	...
	private void abortOffer(Player player, ExchangeOffer offer) {
		synchronized (ExchangeItemLoader.LOCK) {
			...
			((ExchangeItemLoader) GsonHandler.getJsonLoader(ExchangeItemLoader.class)).saveProgress(offer);
	...
	private static ExchangeOffer getOfferBySlot(Player player, int slot) {
		for (ExchangeOffer offer : ((ExchangeItemLoader) GsonHandler.getJsonLoader(ExchangeItemLoader.class)).getOffersList(player.getUsername())) {
