...
import org.scapesoft.game.player.dialogues.impl.TeleportConfirmation;
...
public class TeleportationSelectListener extends EventListener {
	...
	public enum Pages {
		...
		/** City Teleports */
		CITY_TELEPORTS("Varrock", "Edgeville", "Lumbridge", "Falador", "Seers Village", "Ardougne", "Miscellania", "Catherby", "Taverly", BACK) {
			@Override
			public WorldTile[] getTeleportTiles() {
				return new WorldTile[] { TeleportLocations.VARROCK, TeleportLocations.EDGEVILLE, TeleportLocations.LUMBRIDGE, TeleportLocations.FALADOR, TeleportLocations.CAMELOT, TeleportLocations.ARDOUGNE, TeleportLocations.MISCELLANIA, TeleportLocations.CATHERBY, TeleportLocations.TAVERLY };
			}
		},
		/** Skilling */
		SKILL_TRAINING_LOCATIONS("Rogues' Den (Thieving)", "Plank Making (Sawmill)", "Agility Training", "Hunting Jungle", "Falador Mining Dungeon", "Fishing Guild", "Living Rock Cavern", BACK) {
			@Override
			public WorldTile[] getTeleportTiles() {
				return new WorldTile[] { TeleportLocations.ROGUES_DEN, TeleportLocations.PLANK_CREATION_SAWMILL, null, TeleportLocations.HUNTER_TRAINING, TeleportLocations.FALADOR_MINING, TeleportLocations.FISHING_GUILD, TeleportLocations.LIVING_ROCK_CAVERNS };
			}
	...
	public static void npcInteract(Player player) {
		NPC wizard = Utils.findLocalNPC(player, 2328);
		if (wizard == null) {
			return;
		}
		wizard.setNextAnimation(new Animation(811));
		wizard.setNextGraphics(new Graphics(343));
		wizard.setNextForceTalk(new ForceTalk(RANDOM_MESSAGES[Utils.random(RANDOM_MESSAGES.length)].replaceAll("@NAME@", player.getDisplayName())));
	}
	
	private static final String[] RANDOM_MESSAGES = new String[] {
		"@NAME@, enjoy your trip!",
		"Alakazam! @NAME@ goes BLAM!",
		"Where goes @NAME@? Nobody shall know..."
	};
