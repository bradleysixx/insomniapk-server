...
import org.scapesoft.utilities.game.npc.NPCSpawns;
...
public class Region {
		...
		if (getLoadMapStage() == 0) {
			...
			CoresManager.slowExecutor.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						loadRegionMap();
						for (ForceClipping regions : ForceClipping.values()) {
							if (regionId == regions.regionId) {
								for (WorldTile tile : regions.tiles) {
									clip(tile.getPlane(), tile.getX(), tile.getY());
								}
							}
						}
						setLoadMapStage(2);
	...
	public void clip(WorldObject object, int x, int y) {
		if (ObjectRemoval.getCustomObject(object, object.getX(), object.getY(), object.getPlane()) != null) {
		...
		//System.out.println("[type=" + type + ", coords=(" + x + ", " + y + ", " + plane + ")");
	}
	...
	/**
	 * The areas that have force clipping
	 */
	public enum ForceClipping {

		HOME(13110, new WorldTile[] { new WorldTile(3283, 3493, 0), new WorldTile(3283, 3489), new WorldTile(3283, 3488) });

		ForceClipping(int regionId, WorldTile[] tiles) {
			this.regionId = regionId;
			this.tiles = tiles;
		}
