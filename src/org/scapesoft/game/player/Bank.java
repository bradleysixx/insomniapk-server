...
public class Bank implements Serializable {
	...
	public void openBank() {
		...
		player.getPackets().sendIComponentText(762, 31, "22");
		player.getPackets().sendIComponentText(762, 32, "22");
		if (!getPin().hasPin()) {
	...
	public void depositAllInventory(boolean banking) {
		int count = player.getInventory().getItemsContainerSize();
		for (Item item : player.getInventory().getItems().getItems())
			if (item != null && getItem(item.getId()) != null)
				count--;
		if (MAX_BANK_SIZE - count < player.getInventory().getItems().getSize()) {
	...
	public void withdrawItem(int bankSlot, int quantity) {
		if (quantity < 1 || !player.getInterfaceManager().containsInterface(762))
			return;
		...
		if (noted || defs.isStackable()) {
			if (player.getInventory().getItems().containsOne(item)) {
				...
				if (invItem.getAmount() + item.getAmount() <= 0) {
					...
					player.getPackets().sendGameMessage("Not enough space in your inventory.");// osama
				}
	...
	public void addItem(int id, int quantity, int creationTab, boolean refresh) {
		long total = (long) quantity + getNumberOf(id);
		if (id == 995 && total > Integer.MAX_VALUE) {
			long change = total - Integer.MAX_VALUE;
			player.getInventory().addItem(id, (int) change);
			int banked = (int) (Integer.MAX_VALUE - getNumberOf(id));
			if (banked > 0)
				addItem(id, banked, creationTab, refresh);
			return;
		}
