...
public final class LocalPlayerUpdate {
	...
	private static final int MAX_PLAYERS_UPDATED = 0xFF;
	...
	private void processOutsidePlayers(OutputStream stream, OutputStream updateBlockData) {
		...
		stream.finishBitAccess();
		// System.out.println("GPI took " + (System.nanoTime() - start) +
	...
	private void processLocalPlayers(OutputStream stream, OutputStream updateBlockData/*
		...
		stream.initBitAccess();
		int skip = 0;
		int max = localPlayersIndexesCount > MAX_PLAYERS_UPDATED ? MAX_PLAYERS_UPDATED : localPlayersIndexesCount;
		for (int i = 0; i < /* localPlayersIndexesCount */max; i++) {
			...
			if (needsRemove(p)) {
				...
				if (p.hasTeleported()) {
					...
					stream.writeBits(1, 0); // no update needed
					for (int i2 = i + 1; i2 < /* localPlayersIndexesCount */max; i2++) {
	...
	public OutputStream createPacketAndProcess() {
		...
		// System.out.println("LocalPlayerUpdate took " + (System.nanoTime() -
		// start) + " nano seconds for " + player.getUsername());
		return stream;
	}
