...
public final class Inventory implements Serializable {
	...
	public int size() {
		return items.getSize() - items.freeSlots();
	}
	...
	public boolean addItem(Item item) {
		...
		int result = getNumberOf(item.getId()) + item.getAmount();
		if (item.getId() == 995 && (result < 0 || result > Integer.MAX_VALUE)) {
			...
			World.addGroundItem(item, player, player, true, 300);
			return false;
		}
		Item[] itemsBefore = items.getItemsCopy();
		if (!items.add(item)) {
	...
	public void addDroppable(Item item) {
		...
		if (getFreeSlots() == 0) {
			if (contains(item.getId())) {
				if (!item.getDefinitions().isStackable() || (getAmountOf(item.getId()) + item.getAmount() > Integer.MAX_VALUE))
					canAdd = false;
			} else {
				canAdd = false;
			}
