...
public class InterfaceManager {
	...
	public void setOverlay(final int interfaceId) {
	...
	public void sendTaskTab(boolean tab) {
		try {
			...
			int wildernessPlayers = 0;
			for (Player pl : World.getPlayers()) {
				if (pl == null)
					continue;
				if (pl.getControllerManager().getController() instanceof Wilderness) {
					wildernessPlayers++;
				}
			}
			...
			bldr.append("PKers: <col=" + ChatColors.WHITE + ">" + wildernessPlayers + "<br>");
			...
			bldr.append("Slayer Points: <col=" + ChatColors.WHITE + ">" + Utils.format(player.getSlayerManager().getPoints()) + "<br>");
			bldr.append("Achievement Points: <col=" + ChatColors.WHITE + ">" + Utils.format(player.getFacade().getAchievementPoints()) + "<br>");
			bldr.append("Loyalty Points: <col=" + ChatColors.WHITE + ">" + Utils.format(player.getLoyaltyManager().getPoints()) + "<br>");
			bldr.append("Wilderness Points: <col=" + ChatColors.WHITE + ">" + Utils.format(player.getFacade().getWildernessPoints()) + "<br>");
			
			
			if (player.hasDoubleExp()) {
				bldr.append("Bought DXP Left: <col=33CC33>" + TimeUnit.MILLISECONDS.toMinutes(player.getDoubleExpTime() - System.currentTimeMillis()) + "</col> min<br>");
			}
			...
			Collections.sort(players, (o1, o2) -> ((Integer) o1.getRights()).compareTo(o2.getRights()));
			Collections.reverse(players);
			int count = 0;
			for (Player p : players) {
				if (p == null)
					continue;
				if (count >= 30)// staff count
					break;
			...
			/**
			 * Sending all of the text from the StringBuilder onto the
			 * interface.
			 */
