...
public class Bob extends Dialogue {
	...
	public void start() {
		...
		sendNPCDialogue(npcId, ChatAnimation.NORMAL, "Hello boss, I can repair all of your broken", "armour - such as barrows - for a cost.", "Would you like me to do this?");
	}
	...
	private int getRepairCost() {
		...
		for (Item item : player.getInventory().getItems().toArray()) {
			if (item == null)
				continue;
			if (getDegradeType(item.getId()) == null || item.getDefinitions().isNoted() || getFixedId(item) == -1) {
				continue;
			}
	...
	private int getFixedId(Item item) {
		...
		case BARROWS:
			...
			String newName = name.substring(0, name.indexOf("" + charges) - 1);
