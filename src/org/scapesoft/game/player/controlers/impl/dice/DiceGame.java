...
import org.scapesoft.game.player.controlers.impl.dice.DiceSession.Stages;
...
public class DiceGame extends Controller {
	...
	public void openOfferScreen() {
		synchronized (this) {
			...
			player.setCloseInterfacesEvent(() -> closeOffer(CloseType.DECLINED));
		}
	}
	...
	public void closeOffer(CloseType stage) {
		synchronized (this) {
			if (stage == CloseType.NO_SPACE || stage == CloseType.DECLINED) {
	...
	private void giveRewards(Player winner, Player loser) {
		if (winner != null && !World.containsPlayer(winner.getUsername())) {
			System.err.println(winner + " logged out during a dice duel.");
			if (loser != null) {
				returnStake(loser);
				closeInterfaces(loser);
				loser.unlock();
			}
			return;
		}
		final ItemsContainer<Item> rewards = new ItemsContainer<>(46, false);
		Stream.concat(Stream.of(winner.getDiceSession().getStake().getItems()), Stream.of(loser.getDiceSession().getStake().getItems())).forEach(item -> {
		});
		Item[] loserStake = loser.getDiceSession().getStake().getItems();
		Item[] winnerStake = winner.getDiceSession().getStake().getItems();
		concat(of(loserStake), of(winnerStake)).filter(item -> item != null).forEach(item -> {
			if (!rewards.add(item)) {
				winner.getPackets().sendGameMessage("There were too many " + item.getName() + "s in the spoils; they remains are on the floor.");
				World.addGroundItem(item, winner, winner, true, 300);
			}
		});
		...
		winner.setCloseInterfacesEvent(new Runnable() {
			...
			@Override
			public void run() {
				if (w.getDiceSession().isGaveItems())
					return;
				for (Item item : rewards.getItems()) {
					if (item == null)
						continue;
					w.getInventory().addDroppable(item);
				}
				w.getDiceSession().setGaveItems(true);
			}
		});
		if (player != null) {
			player.unlock();
	...
	private void rollDice(boolean change, Player... players) {
		if (players.length == 2) {
			players[0].setNextFaceEntity(players[1]);
			players[1].setNextFaceEntity(players[0]);
		}
