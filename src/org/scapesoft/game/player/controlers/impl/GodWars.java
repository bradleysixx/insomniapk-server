...
import org.scapesoft.game.player.actions.Smithing;
...
public class GodWars extends Controller {
	...
	public boolean logout() {
		setArguments(new Object[] { player.getGodwarsCounts(), lastPrayerRecharge });
	...
	public boolean processObjectClick1(final WorldObject object) {
		if (object.getId() == 57225) {
			...
			if (player.getGodwarsCounts()[4] >= 40 || withinBankArea || hasCerimonial) {
				...
				if (!withinBankArea) {
					if (player.getGodwarsCounts()[4] >= 40) {
						player.getGodwarsCounts()[4] -= 40;
					}
					refresh();
				}
			...
			if (player.getGodwarsCounts()[index] >= requiredKc) {
				...
				player.getGodwarsCounts()[index] -= requiredKc;
				refresh();
			} else {
	...
	public boolean processObjectClick2(WorldObject object) {
		if (!object.withinDistance(player.getLocation(), 5))
			return false;
		if (object.getId() == 26287 || object.getId() == 26286 || object.getId() == 26288 || object.getId() == 26289) {
			...
			player.getPackets().sendGameMessage("The gods pitty you and allow you to leave the encampment.");
	...
	public void incrementKillCount(int index) {
		player.getGodwarsCounts()[index]++;
		refresh();
	}

	public void resetKillCount(int index) {
		player.getGodwarsCounts()[index] = 0;
		refresh();
	}

	public void refresh() {
		player.getVarsManager().sendVarBit(3939, player.getGodwarsCounts()[1]); // arma
		player.getVarsManager().sendVarBit(3941, player.getGodwarsCounts()[0]); // bando
		player.getVarsManager().sendVarBit(3938, player.getGodwarsCounts()[2]);// sara
		player.getVarsManager().sendVarBit(3942, player.getGodwarsCounts()[3]);// zamy
		player.getVarsManager().sendVarBit(8725, player.getGodwarsCounts()[4]);
	}
	...
	public void forceClose() {
		...
		//player.godwarsCounts = new int[5];
	}
