...
import org.scapesoft.game.npc.bosses.glacor.Glacyte;
import org.scapesoft.game.npc.familiar.Familiar;
import org.scapesoft.game.npc.familiar.Steeltitan;
import org.scapesoft.game.npc.godwars.zaros.NexMinion;
...
public final class PlayerCombat extends Action {
	...
	public int processWithDelay(Player player) {
		...
		if (Boolean.TRUE.equals(player.getTemporaryAttributtes().get("miasmic_effect")))
			multiplier = 1.5;
		...
		if (!Utils.isInRange(player.getX(), player.getY(), size, target.getX(), target.getY(), target.getSize(), maxDistance))
			return 0;
	...
	private int getRangeCombatDelay(int weaponId, int attackStyle) {
		int delay = 6;
		if (weaponId != -1) {
			String weaponName = ItemDefinitions.getItemDefinitions(weaponId).getName().toLowerCase().trim();
			if (weaponName.contains("shortbow") || weaponName.contains("karil's crossbow")) {
				delay = 3;
			} else if (weaponName.contains("crossbow")) {
				delay = 5;
			} else if (weaponName.contains("knife") || weaponName.contains("chinchompa")) {
				delay = 2;
			} else if (weaponName.contains("dart")) {
				delay = 3;
			} else if (weaponName.equals("hexhunter bow")) {
				delay = 3;
			} else {
				switch (weaponId) {
				case 6522:
	...
	private int getProjectileDelay(Entity entity, Entity victim) {
		double delay = -1;
		int totalTiles = (int) entity.getLocation().getDistance(victim.getLocation());
		for (int tiles = 0; tiles < totalTiles; tiles++) {
			if (tiles <= 4) {
				delay = 3;
	...
	public int mageAttack(final Player player, int spellId, boolean autocast) {
		...
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			...
			case 25: // air strike
				...
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, baseDamage)));
				World.sendProjectile(player, target, 2699, 38, 28, 50, 50, 0, 0);
				return 5;
			case 28: // water strike
				...
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, baseDamage)));
				World.sendProjectile(player, target, 2703, 30, 28, 50, 50, 0, 0);
				return 5;
			case 36:// bind
				...
				World.sendProjectile(player, target, 178, 38, 18, 50, 70, 0, 0);
			...
			case 30: // earth strike
				...
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, baseDamage)));
				World.sendProjectile(player, target, 2718, 50, 42, 50, 50, 0, 0);
				return 5;
			case 32: // fire strike
				...
				World.sendProjectile(player, target, 2729, 38, 28, 50, 50, 0, 0);
				return 5;
			case 34: // air bolt
			...
			case 39: // water bolt
				...
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, baseDamage)));
				World.sendProjectile(player, target, 2704, 30, 28, 50, 50, 0, 0);
				return 5;
			case 42: // earth bolt
				...
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, baseDamage)));
				World.sendProjectile(player, target, 2719, 50, 42, 50, 50, 0, 0);
				return 5;
			case 45: // fire bolt
				player.setNextGraphics(new Graphics(2728));
			...
			case 52: // water blast
				...
				World.sendProjectile(player, target, 2705, 30, 28, 50, 50, 0, 0);
				return 5;
			case 58: // earth blast
				...
				World.sendProjectile(player, target, 2720, 50, 42, 50, 50, 0, 0);
				return 5;
			case 63: // fire blast
				player.setNextGraphics(new Graphics(2728));
				player.setNextAnimation(new Animation(14223));
				...
				delayMagicHit(2, getMagicHit(player, damage));
				World.sendProjectile(player, target, 2733, 30, 18, 50, 50, 0, 0);
				return 5;
			...
			case 73: // water wave
				...
				World.sendProjectile(player, target, 2706, 30, 28, 50, 50, 0, 0);
				return 5;
			case 77: // earth wave
				...
				World.sendProjectile(player, target, 2721, 50, 42, 50, 50, 0, 0);
				return 5;
			case 80: // fire wave
				player.setNextGraphics(new Graphics(2728));
				player.setNextAnimation(new Animation(14223));
				...
				delayMagicHit(2, getMagicHit(player, damage));
				World.sendProjectile(player, target, 2735, 38, 28, 50, 50, 0, 0);
				return 5;
			case 86: // teleblock
			...
			case 84:// air surge
				...
				World.sendProjectile(player, target, 462, 38, 28, 50, 50, 0, 0);
				return 5;
			case 87:// water surge
				// player.setNextGraphics(new Graphics(2701));
				...
				World.sendProjectile(player, target, 2707, 30, 28, 50, 50, 3, 0);
				return 5;
			case 89:// earth surge
				...
				World.sendProjectile(player, target, 2722, 50, 42, 50, 50, 0, 0);
				return 5;
			case 91:// fire surge
				player.setNextGraphics(new Graphics(2728));
				...
				delayMagicHit(2, getMagicHit(player, damage));
				World.sendProjectile(player, target, 2735, 38, 28, 50, 50, 3, 0);
				World.sendProjectile(player, target, 2736, 38, 28, 50, 50, 20, 0);
				World.sendProjectile(player, target, 2736, 38, 28, 50, 50, 110, 0);
				return 5;
			case 99: // Storm of armadyl //Sonic and Tyler dumped
				...
				delayMagicHit(1, getMagicHit(player, hit));
				World.sendProjectile(player, target, 1019, 18, 18, 50, 50, 0, 0);
			...
			case 29:// Smoke Blitz
				...
				base_mage_xp = 42;
				max_poison_hit = 40;
				delayMagicHit(3, getMagicHit(player, getRandomMagicMaxHit(player, 230)));
				World.sendProjectile(player, target, 386, 18, 18, 50, 50, 0, 0);
				return 6;
			case 33:// Shadow Blitz
				...
				mage_hit_gfx = 381;
				base_mage_xp = 43;
				delayMagicHit(3, getMagicHit(player, getRandomMagicMaxHit(player, 240)));
				World.sendProjectile(player, target, 380, 18, 18, 50, 50, 0, 0);
				return 6;
			case 25:// Blood Blitz
				player.setNextAnimation(new Animation(1978));
				...
				base_mage_xp = 45;
				blood_spell = true;
				delayMagicHit(3, getMagicHit(player, getRandomMagicMaxHit(player, 250)));
				World.sendProjectile(player, target, 374, 18, 18, 50, 50, 0, 0);
				return 6;
			case 21:// Ice Blitz
				player.setNextGraphics(new Graphics(366));
				...
				freeze_time = 15000;
				magic_sound = 169;
				delayMagicHit(3, getMagicHit(player, getRandomMagicMaxHit(player, 260)));
				if (target.hasWalkSteps())
					World.sendProjectile(player, target, 368, 0, 0, 50, 50, 0, 0);
				return 4;
	...
	private int rangeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 18332: // Magic longbow (sighted)
				...
				World.sendProjectile(player, target, 249, 43, 35, 40 + (getProjectileDelay(player, target) * 5), 30, 0, 0);
				World.sendProjectile(player, target, 249, 43, 35, 65 + (getProjectileDelay(player, target) * 5), 60, 0, 0);
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						if (loop == 0) {
							...
							World.sendProjectile(player, target, 2143, 21, 18, 52, 29, 0, 0);
							delayHit(1, weaponId, attackStyle, getRangeHit(player, getRandomMaxHit(player, weaponId, attackStyle, true, true, 1.0, true)));
						} else if (loop == 1) {
							// player.setNextAnimation(new Animation(12174));
							player.setNextGraphics(new Graphics(2138));
							World.sendProjectile(player, target, 2143, 24, 20, 48, 32, 0, 8);
							delayHit(1, weaponId, attackStyle, getRangeHit(player, getRandomMaxHit(player, weaponId, attackStyle, true, true, 1.0, true)));
							stop();
						}
						loop++;
			...
			case 15704:
				...
				if (ammoId == 11212) {
					...
					if (damage2 < 80)
						damage2 = 80;
					World.sendProjectile(player, target, 1099, 44, 18, 35, 53, 3, 0);
					World.sendProjectile(player, target, 1099, 44, 28, 24, 58, 20, 0);
					...
					if (damage2 < 50)
						damage2 = 50;
					World.sendProjectile(player, target, 1101, 44, 28, 40, 35, 3, 0);
					World.sendProjectile(player, target, 1101, 44, 28, 32, 35, 12, 0);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, damage));
					delayHit(3, weaponId, attackStyle, getRangeHit(player, damage2));
				}
				dropAmmo(player, 2);
			...
			case 14684: // zanik cbow
				...
				World.sendProjectile(player, target, 2001, 41, 41, 41, 52, 0, 0);
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("throwing axe") || weaponName.contains("knife") || weaponName.contains("dart") || weaponName.contains("javelin") || weaponName.contains("thrownaxe")) {
					...
					World.sendProjectile(player, target, getKnifeProjectileId(weaponId), 46, 38, 39, 36, 0, 0);
					...
					World.sendProjectile(player, target, 27, 46, 36, 41, 46, 0, 0);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, damage));
					if (weaponId != 4740)
						dropAmmo(player);
					else
					...
					World.sendProjectile(player, target, 909, 32, 22, 38, 27, 2, 0);
					...
					if (Utils.getRandom(player.getSkills().getLevel(Skills.FIREMAKING) << 1) == 0) {
						...
						player.setNextGraphics(new Graphics(2138));
						World.sendProjectile(player, target, 2143, 23, 18, 52, 32, 0, 0);
						delayHit(1, weaponId, attackStyle, getRangeHit(player, getRandomMaxHit(player, weaponId, attackStyle, true)));
						dropAmmo(player, -2);
					}
					...
					World.sendProjectile(player, target, 249, 41, 35, distance > 4 ? 42 : 28, 48, 0, 0);
					int hit = getRandomMaxHit(player, weaponId, attackStyle, true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit));
					checkSwiftGlovesEffect(player, 2, attackStyle, weaponId, hit, 249, 41, 36, 30, 44, 0, 0);
				} else if (weaponId == 21365) { // Bolas
					dropAmmo(player, -3);
					player.setNextAnimation(new Animation(3128));
					World.sendProjectile(player, target, 468, 37, 24, 41, 32, 0, 0);
					int delay = 15000;
					if (target instanceof Player) {
						Player p = (Player) target;
					...
					player.setNextGraphics(new Graphics(getArrowThrowGfxId(weaponId, ammoId), 0, 100));
					// sendProjectile(Entity shooter, Entity receiver, int
					// gfxId, int startHeight, int endHeight, int speed, int
					// delay, int curve, int startDistanceOffset)
					World.sendProjectile(player, target, getArrowProjectileGfxId(weaponId, ammoId), 41, 36, distance > 4 ? 42 : 30, 44, 0, 0);
					// System.out.println(distance);
					int hit = getRandomMaxHit(player, weaponId, attackStyle, true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit));
					if (weaponId == 17295)
						returnHit(target, 3, getMagicHit(player, Utils.random(Math.floor(hit * .5))));
					...
					if (weaponId == 11235 || weaponId == 15701 || weaponId == 15702 || weaponId == 15703 || weaponId == 15704) { // dbows
						World.sendProjectile(player, target, getArrowProjectileGfxId(weaponId, ammoId), 41, 35, 25, 55, 21, 0);
	...
	public static int getKnifeProjectileId(int weapon) {
		switch (weapon) {
		case 806:
			return 226;
		case 807:
		...
		return getKnifeThrowGfxId(weapon);
	}
	...
	public static int getKnifeThrowGfxId(int weaponId) {
		...
		if (weaponId == 806) {
			...
			return 236;
		} else if (weaponId == 811 || weaponId == 5641) {
			return 237;
	...
	private int meleeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 10887: // anchor
				...
				delayHit(2, weaponId, attackStyle, getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, false, 1.0, true)));
				break;
			...
			case 20821: // thok's sword
				...
				Hit fhit = new Hit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, .25, true), HitLook.MELEE_DAMAGE).setCriticalMark();
				Hit shit = new Hit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, .5, true), HitLook.MELEE_DAMAGE).setCriticalMark();
				Hit thit = new Hit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, 1, true), HitLook.MELEE_DAMAGE).setCriticalMark();
				delayHit(0, weaponId, attackStyle, fhit);
				delayHit(1, weaponId, attackStyle, shit);
				delayHit(2, weaponId, attackStyle, thit);
				break;
			default:
				player.getPackets().sendGameMessage("This weapon has no special Attack, if you still see special bar please relogin.");
				return combatDelay;
			...
			int delay = 0;
			if (weaponId == 10887 || (weaponName.toLowerCase().contains("maul") && !weaponName.startsWith("Granite")))
				delay = 1;
			delayHit(delay, weaponId, attackStyle, hit);
			player.setNextAnimation(new Animation(getWeaponAttackEmote(weaponId, attackStyle)));
			WorldTasksManager.schedule(new WorldTask() {
	...
	private void returnHit(Entity entity, int delay, Hit... hits) {
		WorldTasksManager.schedule(new WorldTask() {

			@Override
			public void run() {
				Stream.of(hits).forEach(hit -> {
					if (!hit.getSource().isDead() && !entity.isDead() && !target.isDead() && hit.getDamage() != 0)
						entity.applyHit(hit);
				});
				stop();
			}
			...
			public void run() {
				for (Hit hit : hits) {
					...
					if (player.isDead() || player.hasFinished() || target.isDead() || target.hasFinished())
						return;
					if (hit.getDamage() > -1) {
						if (hit.getLook().equals(HitLook.MAGIC_DAMAGE))
							WorldTasksManager.schedule(new WorldTask() {
								@Override
								public void run() {
									target.applyHit(hit);
					...
					if (hit.getLook() == HitLook.RANGE_DAMAGE || hit.getLook() == HitLook.MELEE_DAMAGE) {
						...
						if (splash) {
							...
							if (mage_hit_gfx != 0) {
								target.setNextGraphics(new Graphics(mage_hit_gfx, 0, mage_hit_gfx == 369 || mage_hit_gfx == 1843 || (mage_hit_gfx > 1844 && mage_hit_gfx < 1855) ? 0 : 96));// 367
								// ice
								// blitz
								if (blood_spell)
	...
	public static int getWeaponAttackEmote(int weaponId, int attackStyle) {
		if (weaponId != -1) {
			...
			if (weaponName != null && !weaponName.equals("null")) {
				...
				if (weaponName.equalsIgnoreCase("boxing gloves")) {
					return 3678;
				}
				...
				if (weaponName.contains("2h sword") || weaponName.equals("dominion sword") || weaponName.equals("saradomin sword")) {
					switch (attackStyle) {
					case 2:
						return 7048;
					case 3:
				...
				if (weaponName.equals("thok's sword")) {
	...
	public static int getMeleeCombatDelay(Player player, int weaponId) {
		if (weaponId != -1) {
			...
			// Interval 3.6
			if (weaponName.contains("godsword") || weaponName.contains("warhammer") || weaponName.contains("battleaxe") || weaponName.contains("maul") || weaponName.equals("dominion sword") || weaponName.contains("greataxe") || weaponName.contains("halberd") || weaponName.contains("2h sword") || weaponName.contains("two handed sword") || weaponName.contains("katana")) {
				return 5;
			}
			if (weaponName.equals("thok's sword"))
				return 6;// SLOWEST WEAPON IN GAME!
			// Interval 4.2
			if (weaponName.contains("granite") && weaponName.contains("maul")) {
				return 6;
			}
			// Interval 3.0
			if (weaponName.contains("spear") || weaponName.contains(" sword") || weaponName.contains("longsword") || weaponName.contains("light") || weaponName.contains("hatchet") || weaponName.contains("pickaxe") || weaponName.contains("mace") || weaponName.contains("hasta") || weaponName.contains("warspear") || weaponName.contains("flail") || weaponName.contains("hammers") || weaponName.equalsIgnoreCase("boxing gloves")) {
				return 4;
			}
		}
		switch (weaponId) {
	...
	private boolean checkAll(Player player) {
		...
		if (target instanceof NPC && player.getRegionId() == 12109) {
			player.sendMessage("You cannot damage this npc.");
			return false;
		}
		...
		if ((!player.clipedProjectile(target, maxDistance == 0 && !forceCheckClipAsRange(target))) || !Utils.isInRange(player.getX(), player.getY(), size, target.getX(), target.getY(), target.getSize(), maxDistance)) {
	...
	private static final boolean HUGGING_ENABLED = true;
