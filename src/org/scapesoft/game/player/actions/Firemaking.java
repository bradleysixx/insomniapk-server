...
import org.scapesoft.game.player.content.achievements.impl.InfernoAdzeAchievement;
...
public class Firemaking extends Action {
	...
	public static boolean checkAll(Player player, Fire fire, boolean usingPyre) {
		...
		for (WorldObject object : player.getRegion().getObjects()) {
			if (object.getType() == 22)
				continue;
			if (object.getLocation().equals(player.getWorldTile())) {
				player.getPackets().sendGameMessage("You can't light a fire here.");
				return false;
			}
		}
