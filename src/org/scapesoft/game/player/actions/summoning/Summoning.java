package org.scapesoft.game.player.actions.summoning;
...
import org.scapesoft.game.minigames.clanwars.ClanWars.Rules;
...
public class Summoning {
	...
	public static Familiar createFamiliar(Player player, Pouches pouch) {
		try {
			return (Familiar) Class.forName("org.scapesoft.game.npc.familiar." + (NPCDefinitions.getNPCDefinitions(getNPCId(pouch.getRealPouchId()))).getName().replace(" ", "").replace("-", "").replace("(", "").replace(")", "")).getConstructor(Player.class, Pouches.class, WorldTile.class, int.class, boolean.class).newInstance(player, pouch, player, -1, true);
		} catch (Throwable e) {
			e.printStackTrace();
			return null;
		}
