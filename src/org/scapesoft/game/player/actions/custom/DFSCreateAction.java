package org.scapesoft.game.player.actions.custom;

import org.scapesoft.game.Animation;
import org.scapesoft.game.player.Player;
import org.scapesoft.game.player.Skills;
import org.scapesoft.game.player.actions.Action;
import org.scapesoft.game.player.dialogues.ItemMessage;
...
public class DFSCreateAction extends Action {
	...
	public int processWithDelay(Player player) {
		...
		case 10:
			...
			player.getSkills().addExpNoModifier(Skills.SMITHING, 50000);
			player.stopAll();

			player.getDialogueManager().startDialogue(ItemMessage.class, 11283,
		...
		if (player.getSkills().getLevelForXp(Skills.SMITHING) < 90) {
			player.getDialogueManager()
					.startDialogue(SimpleMessage.class,
							"You need a smithing level of 90 to smith a dragon fire shield.");
			return false;
		}
		if (!player.getInventory().containsItems(
				new int[] { 11286, 2347, 1540 }, new int[] { 1, 1, 1 })) {
			player.getDialogueManager().startDialogue(SimpleMessage.class,
					"You need to have a visage, hammer, and anti-",
					"dragon fire shield in your inventory to do this.");
			return false;
		}
		return true;
	}
