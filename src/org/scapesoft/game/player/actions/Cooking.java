...
import org.scapesoft.game.player.content.achievements.impl.CookingGauntletsMediumAchievement;
import org.scapesoft.game.player.content.wild.WildernessActivityManager;
import org.scapesoft.game.player.content.wild.activities.FCActivity;
...
public class Cooking extends Action {
	...
	/**
	 * Gets the amount of experience to give player, changing if wilderness activity is the firemaking and cooking one
	 * 
	 * @param player
	 *            The player
	 * @return
	...
	public int processWithDelay(Player player) {
		...
		if ((player.getSkills().getLevel(Skills.COOKING) >= cook.getBurningLvl()) ? false : isBurned(cook, player)) {
			...
			player.getSkills().addXp(cook.getRawItem().getId() == Cookables.HARDENED_STRAIT_ROOT.getRawItem().getId() ? Skills.FIREMAKING : Skills.COOKING, getExp(player));
