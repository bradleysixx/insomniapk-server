...
import org.scapesoft.api.event.listeners.interfaces.PlankCreationListener.Planks;
...
import org.scapesoft.game.player.actions.Smelting.SmeltingBar;
...
public class Magic {
	...
	public static final boolean hasInfiniteRunes(int runeId, int weaponId, int shieldId) {
		if (runeId == AIR_RUNE) {
			...
			if (weaponId == 1387 || weaponId == 1401 || weaponId == 1393 || weaponId == 6562) {
				return true;
			}
		}
		return false;
	...
	public static final void processNormalSpell(Player player, int spellId, int packetId) {
		...
		case 50:
			...
			List<SmeltingBar> bars = SmeltingBar.getBarsContaining(target.getId());
			if (bars.size() == 0) {
				player.sendMessage("You cannot superheat this item.");
				return;
			}
			SmeltingBar barToHeat = null;
			for (SmeltingBar bar : bars) {
				boolean containsOres = true;
				j: for (Item item : bar.getItemsRequired()) {
			...
			if (barToHeat == null) {
				player.sendMessage("Error superheating this item, report on forums!");
				return;
			}

			if (!player.getInventory().containsItem(barToHeat.getItemsRequired()[0].getId(), barToHeat.getItemsRequired()[0].getAmount())) {
				player.getDialogueManager().startDialogue(SimpleMessage.class, "You need " + barToHeat.getItemsRequired()[0].getDefinitions().getName() + " to create a " + barToHeat.getProducedBar().getDefinitions().getName() + ".");
				return;
			}
			if (barToHeat.getItemsRequired().length > 1) {
				if (!player.getInventory().containsItem(barToHeat.getItemsRequired()[1].getId(), barToHeat.getItemsRequired()[1].getAmount())) {
					player.getDialogueManager().startDialogue("SimpleMessage", "You need " + barToHeat.getItemsRequired()[1].getDefinitions().getName() + " to create a " + barToHeat.getProducedBar().getDefinitions().getName() + ".");
					return;
				}
			}
			if (player.getSkills().getLevel(Skills.SMITHING) < barToHeat.getLevelRequired()) {
				player.getDialogueManager().startDialogue("SimpleMessage", "You need a Smithing level of at least " + barToHeat.getLevelRequired() + " to smelt " + barToHeat.getProducedBar().getDefinitions().getName());
				return;
			}
			for (Item required : barToHeat.getItemsRequired()) {
				player.getInventory().deleteItem(required.getId(), required.getAmount());
			}
			player.getInventory().addItem(barToHeat.getProducedBar());
			player.setNextAnimation(new Animation(725));
			player.setNextGraphics(new Graphics(148, 0, 110));
			player.getSkills().addXp(Skills.SMITHING, barToHeat.getExperience());
		...
		case 59: // high alch
			...
			player.lock(4);
			player.setActionTime(player.getActionTime() + TimeUnit.SECONDS.toMillis(Utils.random(5, 20)));
			player.getInterfaceManager().openGameTab(7);
			...
			player.getInventory().addItem(new Item(995, ancientStat ? 1000000 : (int) (target.getDefinitions().getValue() * (highAlch ? 0.6D : 0.3D))));
	...
	private static void useHomeTele(Player player) {
		...
		if (player.isLocked() || player.getAttackedByDelay() + 10000 > currentTime || player.getEmotesManager().getNextEmoteEnd() >= currentTime) {
	...
	public static final boolean checkRunes(Player player, boolean delete, int... runes) {
		if (Constants.isVPS && player.getRights() == Rights.OWNER.ordinal())
			return true;
		int weaponId = player.getEquipment().getWeaponId();
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (player.getTeleportationEvent() != null) {
					...
					player.setTeleportationEvent(null);
				}
				player.stopAll();
			}
		}, delay);
	...
	public static boolean useTabTeleport(final Player player, final int itemId) {
		...
		if (itemId == 8013) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					if (!teled) {
						...
						if (player.getHouse().shouldArriveInPortal()) {
							player.setNextWorldTile(POHLocation.HOME_PORTAL.getTile());
						} else {
			...
			return true;
		} else if (useTeleTab(player, TABS[itemId - 8007])) {
			player.getInventory().deleteItem(itemId, 1);
		}
		return true;
	}
