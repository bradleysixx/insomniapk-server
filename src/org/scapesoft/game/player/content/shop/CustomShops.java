...
public enum CustomShops {
	...
	GOLDPOINTS(new String[] { "Gold_Points_Only", "Gold_Points_Armours", "Gold_Points_Weapons", "Gold_Points_Untradeables", "Gold_Points_Rares" }) {
		@Override
		public void sendCost(Player player, Item item) {
		...
		public void updateShopCurrency(Player player) {
			player.getPackets().sendIComponentText(INTERFACE_ID, 24, "Gold Points: " + player.getFacade().getGoldPoints());
		}
	},
	...
	WildyPoints(new String[] { "Wilderness_Point_Rewards" }) {
		@Override
		public void sendCost(Player player, Item item) {
			sendCustomCost(player, ShopPrices.WILDY_POINT_SHOP, item, "Wilderness Points");
		}

		@Override
		public boolean buyCustomCurrency(Player player, Item item, int buyQuantity) {
			return buyWith(player, "Wilderness Points", item, buyQuantity, player.getFacade().getWildernessPoints(), ShopPrices.WILDY_POINT_SHOP);
		}

		@Override
		public boolean onBuy(Player player, Item item, int buyQuantity, int customPrice) {
			if (player.getInventory().addItem(item.getId(), item.getId() == 12852 ? buyQuantity : item.getAmount())) {
				player.getFacade().setWildernessPoints(player.getFacade().getWildernessPoints() - customPrice);
				return true;
			}
			return false;
		}
		...
		@Override
		public void updateShopCurrency(Player player) {
			player.getPackets().sendIComponentText(INTERFACE_ID, 24, "Wilderness Points: " + player.getFacade().getWildernessPoints());
		}
	},
	...
	private static final int INTERFACE_ID = 620;
