...
public class Prayer implements Serializable {
	...
	private boolean usePrayer(int prayerId) {
		...
		if (getPrayerBook() == 1 && (prayerId >= 18)) {
			/**
			 * Players must have voted 3 or more times to have access to
			 * soulsplit & turmoil. If you are a member you dont have this
			 * restriction.
			 */
			if (!player.isDonator() && player.getFacade().getTimesVoted() < 3) {
				player.getDialogueManager().startDialogue(SimpleItemMessage.class, 13050, "You need to have voted 3 or more times", "to unlock this prayer.");
				return false;
			}
		}
		if (player.getCurrentFriendChat() != null) {
