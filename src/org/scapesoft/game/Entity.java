...
import org.scapesoft.cache.loaders.AnimationDefinitions;
...
public abstract class Entity extends WorldTile {
	...
	public volatile transient WorldTile lastWorldTile;
	private volatile transient WorldTile nextWorldTile;
	...
	private volatile transient boolean teleported;
	...
	public Player getMostDamageReceivedSourcePlayer() {
		Player player = null;
		int damage = -1;
		Iterator<Entry<Entity, Integer>> it$ = this.getReceivedDamage().entrySet().iterator();
		while(it$.hasNext()) {
	...
	public boolean addWalkStep(int nextX, int nextY, int lastX, int lastY, boolean check) {
		...
		// prevent npcs from walking on top of each other
		if (this instanceof NPC) {
			if (!World.canMoveNPC(getPlane(), nextX, nextY, getSize()))
				return false;
			for (int index : getRegion().getNPCsIndexes()) {
				NPC npc = World.getNPCs().get(index);
				if (npc == null || npc.getLocation() == null || npc == this)
					continue;
				if (nextX == npc.getX() && nextY == npc.getY())
					return false;
				/*				if (npc.getX() == getX() && npc.getY() == getY())
									return false;*/
			}
		}
	...
	public StringBuilder processEntity() {
		...
		builder.append("processPoison() took: " + w.elapsed() + "ms\n");
	...
	public void processReceivedHits() {
		if (isPlayer()) {
			if (getReceivedHits().size() < 5 && (player().getEmotesManager().getNextEmoteEnd() >= Utils.currentTimeMillis())) {
				return;
			}
		}
		Hit hit;
	...
	public boolean canWalkNPC(int toX, int toY) {
		...
		return id == 6260 || id == 6222 || id == 6203 || id == 6247 || id == 8133 || canWalkNPC(toX, toY, true);// false
	}
	...
	/**
	 * @param speakingWith the speakingWith to set
	 */
	...
	/**
	 * @param receivedHits the receivedHits to set
	 */
	...
	/**
	 * @param receivedDamage the receivedDamage to set
	 */
