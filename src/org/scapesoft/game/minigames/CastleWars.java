...
import org.scapesoft.game.npc.others.CastleWarBarricade;
import org.scapesoft.game.player.Equipment;
import org.scapesoft.game.player.Player;
import org.scapesoft.game.player.content.Foods.Food;
import org.scapesoft.game.player.controlers.impl.castlewars.CastleWarsPlaying;
import org.scapesoft.game.player.dialogues.CastleWarsScoreboard;
...
public final class CastleWars {
	...
	public static void joinPortal(Player player, int team) {
		...
		if (playingGame == null && waiting[team].size() >= playersNeededToStart) {
			...
			refreshTimeLeft(player); // You cannot take non-combat items into
										// the arena
	}
	...
	public static void handleInterfaces(Player player, int interfaceId, int componentId, int packetId) {
		if (interfaceId == 60 && componentId != 1 && componentId != 5 && componentId != 9)
	...
	public static void interactCastleWarsTicketExchange(Player player, int componentId, boolean buy) {
		...
		} else player.getPackets().sendGameMessage(ItemExamines.getExamine(new Item(id)));
	}
