...
import org.scapesoft.engine.process.impl.SwitchingProcessor;
import org.scapesoft.engine.process.impl.SwitchingProcessor.ItemSwitch;
...
import org.scapesoft.game.npc.familiar.Familiar.SpecialAttack;
...
import org.scapesoft.game.player.actions.BoxAction;
import org.scapesoft.game.player.actions.Firemaking;
...
import org.scapesoft.game.player.actions.Herblore;
...
import org.scapesoft.game.player.actions.Hunter.FlyingEntities;
import org.scapesoft.game.player.actions.Woodcutting.Nest;
import org.scapesoft.game.player.actions.prayer.Bone;
import org.scapesoft.game.player.actions.prayer.Burying;
import org.scapesoft.game.player.actions.summoning.Pouches;
import org.scapesoft.game.player.actions.summoning.Summoning;
import org.scapesoft.game.player.content.AncientEffigies;
import org.scapesoft.game.player.content.ArmourSets;
import org.scapesoft.game.player.content.Foods;
...
import org.scapesoft.game.player.dialogues.Transportation;
import org.scapesoft.game.player.dialogues.impl.Bob;
import org.scapesoft.game.player.dialogues.impl.NameChangeRequest;
...
public class InventoryOptionsHandler {
	...
	public static void handleItemOption1(Player player, final int slotId, final int itemId, Item item) {
		...
		if (itemId == AncientEffigies.SATED_ANCIENT_EFFIGY || itemId == AncientEffigies.GORGED_ANCIENT_EFFIGY || itemId == AncientEffigies.NOURISHED_ANCIENT_EFFIGY || itemId == AncientEffigies.STARVED_ANCIENT_EFFIGY) {
		...
		} else if (itemId == CoinCaskets.REWARDS.getItemId()) {
			CoinCaskets casket = CoinCaskets.REWARDS;
			...
			return;
		} else if (itemId == CoinCaskets.REGULAR.getItemId()) {
			CoinCaskets casket = CoinCaskets.REGULAR;
			player.getInventory().deleteItem(slotId, item);
			int coinAmount = casket.getBaseCoinAmount();
			int extra = Utils.random(1000, casket.getExtraCoinAmount());
			player.getInventory().addDroppable(new Item(995, coinAmount + extra));
	...
	public static void handleItemOnItem(final Player player, InputStream stream) {
		...
		if (interfaceId2 == 430 && interfaceId == Inventory.INVENTORY_INTERFACE) {
			if (toSlot >= 28)
				return;
			long time = Utils.currentTimeMillis();
			...
			if (player.getLockDelay() >= time || player.getEmotesManager().getNextEmoteEnd() >= time)
				return;
			Magic.processLunarSpell(player, component, (byte) toSlot);
		}
		...
		if (interfaceId == Inventory.INVENTORY_INTERFACE && interfaceId == interfaceId2 && !player.getInterfaceManager().containsInventoryInter()) {
			...
			if (ItemOnTypeHandler.handleItemOnItem(player, itemUsed, usedWith)) {
				...
				return;
			} else if (contains(590, 6055, itemUsed, usedWith)) {
				WorldTasksManager.schedule(new WorldTask() {
					@Override
					public void run() {
						player.setNextForceTalk(new ForceTalk("Smoke weed every day!"));
						player.getInventory().removeItems(new Item(6055));
						stop();
					}
				}, 2);
	...
	public static void handleItemOption6(Player player, int slotId, int itemId, Item item) {
		...
		if (pouches != null) {
			...
			player.getDialogueManager().startDialogue(Transportation.class, "Duel Arena", TeleportLocations.DUEL_ARENA, "Castle Wars", TeleportLocations.CASTLE_WARS, "Mobilising Armies", new WorldTile(2411, 2841, 0), "Home", Constants.HOME_TILE, itemId);
	...
	public static void handleItemOnNPC(Player player, NPC npc, Item item) {
		...
		if (ItemOnTypeHandler.handleItemOnNPC(player, npc, item)) {
			player.turnTo(npc);
			return;
		}
