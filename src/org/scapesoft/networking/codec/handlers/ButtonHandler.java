...
import org.scapesoft.game.player.EmotesManager;
...
import org.scapesoft.game.player.actions.Rest;
import org.scapesoft.game.player.actions.Smithing.ForgingInterface;
import org.scapesoft.game.player.content.Enchanting;
import org.scapesoft.game.player.content.Magic;
import org.scapesoft.game.player.content.PlayerDeathInformation;
...
import org.scapesoft.game.player.content.Runecrafting.Talismans;
...
import org.scapesoft.game.player.content.shop.Shop;
...
import org.scapesoft.game.player.dialogues.impl.WorldMapConfirmation;
	...
	public static void handleButtons(final Player player, InputStream stream, int packetId) {
				...
				if (packetId == 25) {
					...
					if (player.getEquipment().getShieldId() == 11283 || player.getEquipment().getShieldId() == 11284) {
						...
						if (target != null) {
							...
							World.sendProjectile(player, player, ((PlayerCombat) player.getActionManager().getAction()).getTarget(), 1166, 42, 50, speed, 43, 10, 0);
		...
		if (interfaceId == 548 || interfaceId == 746 || interfaceId == 387) {
			...
			if (componentId == 11) {
				...
				if (capeId == 20771) {
					switch (packetId) {
					case 64: // kandarin monestary
						Magic.sendNormalTeleportSpell(player, 1, 0, new WorldTile(2606, 3220, 0));
						break;
					case 4: // ardougne farm
						Magic.sendNormalTeleportSpell(player, 1, 0, new WorldTile(2668, 3374, 0));
						break;
					case 52: // summmoning restore
						if (player.getFacade().getArdougneOperators()[2] == -1 || TimeUnit.MILLISECONDS.toHours(System.currentTimeMillis() - player.getFacade().getArdougneOperators()[2]) >= 24) {
							player.getSkills().set(Skills.SUMMONING, player.getSkills().getLevelForXp(Skills.SUMMONING));
							player.getSkills().refresh(Skills.SUMMONING);
							player.getFacade().getArdougneOperators()[2] = System.currentTimeMillis();
						} else {
							long h = player.getFacade().getArdougneOperators()[2] + TimeUnit.DAYS.toMillis(1);
							hours = TimeUnit.MILLISECONDS.toHours(h - System.currentTimeMillis());
							player.getDialogueManager().startDialogue(SimpleMessage.class, "You must wait " + hours + " more hours to do this.");
						}
						break;
			...
			if (componentId == 0) {
				...
				if (packetId == 18 && talisman != null) {
					Magic.sendNormalTeleportSpell(player, 0, 0, talisman.getTile());
					player.sendMessage("You teleport to the " + talisman.name().toLowerCase() + " crafting altar.");
					return;
				}
				if (packetId == DefaultGameDecoder.ACTION_BUTTON1_PACKET) {
			...
			if (componentId == 4) {
				...
				synchronized (SwitchingProcessor.LOCK) {
					if (SwitchingProcessor.getSwitchesForPlayer(player, false).size() > 0) {
						...
						switches.forEach((i) -> ButtonHandler.sendWear(i.getPlayer(), i.getSlotId(), i.getItemId()));
					}
				}
			...
			} else if (componentId == 13) { // Report abuse
				player.sendMessage("Feature is currently unavailable.");
