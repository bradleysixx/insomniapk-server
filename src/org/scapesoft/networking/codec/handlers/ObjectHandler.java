...
import org.scapesoft.game.player.actions.BoxAction.HunterEquipment;
...
import org.scapesoft.game.player.actions.Smithing.ForgingBar;
import org.scapesoft.game.player.actions.Smithing.ForgingInterface;
import org.scapesoft.game.player.actions.WaterFilling;
import org.scapesoft.game.player.actions.WildernessObelisks;
import org.scapesoft.game.player.actions.Woodcutting;
import org.scapesoft.game.player.actions.Woodcutting.TreeDefinitions;
import org.scapesoft.game.player.actions.mining.CrashedStarMining;
import org.scapesoft.game.player.actions.mining.CrashedStarMining.CrashedStarDefinitions;
import org.scapesoft.game.player.actions.mining.EssenceMining;
import org.scapesoft.game.player.actions.mining.EssenceMining.EssenceDefinitions;
import org.scapesoft.game.player.actions.mining.Mining;
import org.scapesoft.game.player.actions.mining.Mining.RockDefinitions;
...
import org.scapesoft.game.player.actions.thieving.Thieving;
import org.scapesoft.game.player.content.ItemOnTypeHandler;
import org.scapesoft.game.player.content.Magic;
import org.scapesoft.game.player.content.Runecrafting;
import org.scapesoft.game.player.content.achievements.impl.VarrockEasyDitchAchievement;
import org.scapesoft.game.player.content.agility.Agility;
import org.scapesoft.game.player.content.agility.BarbarianOutpostAgility;
import org.scapesoft.game.player.content.agility.GnomeAgility;
import org.scapesoft.game.player.content.agility.WildernessAgility;
...
import org.scapesoft.game.player.content.wild.activities.MiningActivity;
...
import org.scapesoft.game.player.dialogues.impl.PrayerBoneD;
...
public class ObjectHandler {
	...
	private static void handleOption1(final Player player, final WorldObject object) {
		...
		player.stopAll();
		if (object.getId() == 2283 || object.getId() == 1948) {
			WorldTile targetTile = object.getId() == 2283 ? new WorldTile(3005, 3953, 0) : new WorldTile(2536, 3553, 0);
			player.setRouteEvent(new RouteEvent(targetTile, new Runnable() {
		...
		player.setRouteEvent(new RouteEvent(object, new Runnable() {
			...
			public void run() {
				...
				if (GoblinWars.handleObjects(player, id)) {
					return;
				}
				...
				if (hunterNpc != null) {
					...
					player.getDialogueManager().startDialogue("HomePortal");
					return;
				} else if (object.getId() == 15482) {
					player.getDialogueManager().startDialogue("EnterHouse");
					return;
				} else if (id == 9294) {
				...
				} else if (id == 39830 || id == 9398) { // bank deposit box
					...
					if (OwnedObjectManager.removeObject(player, object)) {
						...
						player.getPackets().sendGameMessage("This isn't your trap.");
				} else if (object.getDefinitions().name.equalsIgnoreCase("Obelisk") && object.getY() > 3527 && object.getDefinitions().getOption(1).equalsIgnoreCase("activate")) {
					WildernessObelisks.initializeTeleportation(player, object);
					...
					if (WildernessActivityManager.getSingleton().isActivityCurrent(MiningActivity.class)) {
						MiningActivity activity = WildernessActivityManager.getSingleton().getWildernessActivity(MiningActivity.class);
						CrashedStar star = activity.getCrashedStar(id);
						if (star == null) {
							return;
						}
						player.getActionManager().setAction(new CrashedStarMining(star, CrashedStarDefinitions.CRASHED_STAR));
					}
				} else if (id == 2478) {
		...
		player.setRouteEvent(new RouteEvent(object, new Runnable() {
			...
			public void run() {
				...
				if (id == 36786 || id == 42378 || id == 42377 || id == 42217 || id == 27663) {
					...
					switch (objectDef.name.trim().toLowerCase()) {
					case "bank booth":
	...
	private static void handleExamine(final Player player, WorldObject object) {
		...
		if (player.getRights() == Rights.OWNER.ordinal() || Constants.DEBUG) {
