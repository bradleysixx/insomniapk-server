...
import org.scapesoft.game.player.actions.PlayerFollow;
		...
		player.setRouteEvent(new RouteEvent(p2, new Runnable() {
			...
			public void run() {
				...
				if (player.getControllerManager().getController() != null) {
					if (!player.getControllerManager().getController().canTrade(p2)) {
						player.getDialogueManager().startDialogue(SimpleMessage.class, "You cannot trade in here.");
						return;
					}
				}
				if (p2.getControllerManager().getController() != null) {
					if (!p2.getControllerManager().getController().canTrade(player)) {
						player.getDialogueManager().startDialogue(SimpleMessage.class, "You cannot trade in here.");
						return;
					}
				}
